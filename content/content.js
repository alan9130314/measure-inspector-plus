...
(() => {
  ...
  // ── SVG Icon set ─────────────────────────────────────────────────────────
  const iconSvg = body =>
    `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" focusable="false"><g stroke="currentColor" stroke-width="1.45" stroke-linecap="round" stroke-linejoin="round">${body}</g></svg>`;
  const IC = {
    inspect: iconSvg('<path d="M5 2.75H3.75a1 1 0 0 0-1 1V5"/><path d="M11 2.75h1.25a1 1 0 0 1 1 1V5"/><path d="M5 13.25H3.75a1 1 0 0 1-1-1V11"/><path d="M8 5.35a2.65 2.65 0 1 0 0 5.3 2.65 2.65 0 0 0 0-5.3Z"/><path d="m10 10 2.4 2.4"/>'),
    guides: iconSvg('<path d="M8 2.5v11"/><path d="M2.5 8h11"/><path d="M4 5.25h2.1"/><path d="M9.9 10.75H12"/><path d="M5.25 4v2.1"/><path d="M10.75 9.9V12"/>'),
    cursor: iconSvg('<path d="M4 2.75v10.5l2.95-2.95 1.75 3.55 1.55-.78-1.72-3.43h4.17L4 2.75Z"/>'),
    snap: iconSvg('<path d="M3.25 4.25v2.3a4.75 4.75 0 0 0 9.5 0v-2.3"/><path d="M3.25 4.25h2.3"/><path d="M10.45 4.25h2.3"/><path d="M8 2.25v2.9"/><path d="M8 10.85v2.9"/>'),
    clear: iconSvg('<path d="m4.25 9.95 5.7-5.7a1.15 1.15 0 0 1 1.63 0l.17.17a1.15 1.15 0 0 1 0 1.63l-5.7 5.7H3.25v-2.8Z"/><path d="m8.75 5.45 1.8 1.8"/><path d="M2.75 13.25h8.5"/>'),
    shortcuts: iconSvg('<rect x="2.25" y="3.25" width="11.5" height="9.5" rx="2"/><path d="M4.75 6.25h.05"/><path d="M7.25 6.25h.05"/><path d="M9.75 6.25h.05"/><path d="M12.25 6.25h.05"/><path d="M4.75 8.75h.05"/><path d="M7.25 8.75h.05"/><path d="M9.75 8.75h2.55"/><path d="M4.75 11.25h6.5"/>'),
    sun: iconSvg('<path d="M8 4.65a3.35 3.35 0 1 0 0 6.7 3.35 3.35 0 0 0 0-6.7Z"/><path d="M8 1.9v1.25"/><path d="M8 12.85v1.25"/><path d="M1.9 8h1.25"/><path d="M12.85 8h1.25"/><path d="m3.65 3.65.9.9"/><path d="m11.45 11.45.9.9"/><path d="m12.35 3.65-.9.9"/><path d="m4.55 11.45-.9.9"/>'),
    moon: iconSvg('<path d="M10.35 2.55a5.7 5.7 0 1 0 3.1 8.8 4.9 4.9 0 0 1-3.1-8.8Z"/>'),
    panelHide: iconSvg('<rect x="2.5" y="3" width="11" height="8" rx="1.8"/><path d="M5 6.75h6"/><path d="m6 9 2 1.8L10 9"/>'),
    settings: iconSvg('<path d="M3 4.5h5.35"/><path d="M11.15 4.5H13"/><path d="M3 8h1.85"/><path d="M7.65 8H13"/><path d="M3 11.5h5.35"/><path d="M11.15 11.5H13"/><path d="M8.35 3.25v2.5"/><path d="M4.85 6.75v2.5"/><path d="M8.35 10.25v2.5"/>'),
    cycleEdit: iconSvg('<path d="M3.75 9.95 9.4 4.3l2.3 2.3-5.65 5.65h-2.3v-2.3Z"/><path d="m8.35 5.35 2.3 2.3"/><path d="M11.15 2.75h.55a1.55 1.55 0 0 1 1.55 1.55v.55"/><path d="M4.85 13.25H4.3a1.55 1.55 0 0 1-1.55-1.55v-.55"/>'),
  };
  ...
  function redraw() {
    ...
    if (S.mode === MODE_INSPECTOR) {
      if (S.selected.length > 0) {
        ...
        S.selected.forEach(el => drawFlexGridGapOverlay(el));
  ...
  function marginBoxFor(el) {
    let css;
    try { css = window.getComputedStyle(el); } catch (_) { return null; }
    const r = el.getBoundingClientRect();
    if (r.width === 0 && r.height === 0) return null;
    const ml = parseFloat(css.marginLeft)   || 0;
    const mr = parseFloat(css.marginRight)  || 0;
    const mt = parseFloat(css.marginTop)    || 0;
    const mb = parseFloat(css.marginBottom) || 0;
    return {
      left:   r.left   - ml,
      top:    r.top    - mt,
      right:  r.right  + mr,
  ...
  /**
   * Cluster child margin rects into flex lines / grid tracks by cross-axis overlap.
   * Within each line, every item's hole is expanded to the line's full cross-axis
   * extent so that leftover cross-axis space (e.g. below a shorter item with
   * align-items: flex-start) is treated as item-occupied, not gap.
   */
  function buildLineExpandedHoles(rects, isRowAxis) {
    ...
    for (const r of sorted) {
      const cs0 = isRowAxis ? r.top   : r.left;
      const ce0 = isRowAxis ? r.bottom: r.right;
      let merged = null;
      for (const ln of lines) {
        const ov = cs0 < ln.crossEnd && ce0 > ln.crossStart;
        if (ov) { merged = ln; break; }
      }
      if (merged) {
        merged.items.push(r);
        merged.crossStart = Math.min(merged.crossStart, cs0);
        merged.crossEnd   = Math.max(merged.crossEnd,   ce0);
      } else {
        lines.push({ items: [r], crossStart: cs0, crossEnd: ce0 });
      }
    }
    const holes = [];
    for (const ln of lines) {
      for (const r of ln.items) {
        holes.push(isRowAxis
          ? { left: r.left,        top:  ln.crossStart, right: r.right,       bottom: ln.crossEnd }
          : { left: ln.crossStart, top:  r.top,         right: ln.crossEnd,   bottom: r.bottom    }
        );
      }
    }
    return holes;
